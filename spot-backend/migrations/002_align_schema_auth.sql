-- Align existing empty schema_auth tables (historical migration).
-- Final shape after 003: gender on users, no user_profiles.

DROP TABLE IF EXISTS schema_auth.otp_tokens CASCADE;
DROP TABLE IF EXISTS schema_auth.otp_verifications CASCADE;
DROP TABLE IF EXISTS schema_auth.user_profiles CASCADE;
DROP TABLE IF EXISTS schema_auth.users CASCADE;
...
CREATE TABLE schema_auth.otp_verifications (
