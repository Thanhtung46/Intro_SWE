...
| `spot-backend/` | Node.js/Express, domain-driven (controller/dto/entity/repository/service), ESM, Node ≥ 18 | **Runnable** (local or Docker). Auth + **matchmaking (kèo) Phases 1–5** + **Groups (hội) G0–G5** + **Tournaments (giải đấu) T0–T5** done. Default DB is **Supabase Postgres** (Session pooler), not the optional compose `postgres` profile. See `spot-backend/AGENTS.md`. |
| `spot-ai-services/{recommendation,nlp-assistant}/` | Python/FastAPI | **Implemented** — real app code, `requirements.txt`, `Dockerfile`, tests. `noshow-prediction/` is still an **empty folder scaffold** (`app/`, `models/`, `services/`, `data/` — no code). See `spot-ai-services/AGENTS.md`. |
...
| `spot-frontend-mobile/` | Expo, React Native, expo-router, Zustand, Axios — `package.json` currently pins Expo `^57`/React Native `^0.86`/React `19.2.8` (not Expo 49/RN 0.72 as this line used to say; version drifts fast here, so check `package.json` directly). See `spot-frontend-mobile/AGENTS.md` for current status — it has a real, mostly-wired auth/onboarding/home flow, not an empty scaffold, and plain `npm install` works (no `--legacy-peer-deps` needed anymore). |
...
[`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) (section Matchmaking).
...
**Aug 2026 (P0–P3):** expiry worker, Completed tab = full+ended only, Manage Squad payment fields, `pendingCount`, host review — see [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) **Changelog bảo trì** and [`spot-backend/docs/API.md`](./spot-backend/docs/API.md) changelog block.

**Figma Homepage 1 (`95:2675` / list `95:2417`) — BE done**

Locked after product review. Detail: [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md)
...
filter `87:1903`) — BE endpoints exist; see spot-backend Codex **Figma** table.

**Figma Manage Matches (`101:98`) — product locked (BE done)**

Full detail: [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) section **Manage Matches 101:98**.
...
**Manage Matches** (`101:98`), **Manage Groups** (`101:2`), và **Manage Tournaments** (mirror Groups FAB) là các màn riêng — đừng trộn route/tab. Groups BE: [`spot-backend/docs/API.md`](./spot-backend/docs/API.md) §8; Tournaments: §9; agent map: [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md).

Hide verified on kèo host profile; hide Booking chrome on filter sheet.

### Referee (trọng tài) — BE done, FE to wire (Aug 2026)

**Docs:** [`spot-backend/docs/REFEREE_PLAN.md`](./spot-backend/docs/REFEREE_PLAN.md) (product) · [`spot-backend/docs/API.md`](./spot-backend/docs/API.md) §8.1, §9.2, Referee endpoints · [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md).
...
Full detail: [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) section **Host form 99:2**.
...
[`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) section **Groups (hội)**.
...
Agent + API: [`spot-backend/AGENTS.md`](./spot-backend/AGENTS.md) **Tournaments (giải đấu)** · [`spot-backend/docs/API.md`](./spot-backend/docs/API.md) §9.
...
- **`spot-backend` is runnable** (auth + matchmaking + groups). Read `spot-backend/AGENTS.md` before changing kèo or groups APIs. Compose from this directory, not from `spot-backend/`.
...
- Each scaffolded app under `spot-*/` now has its own `AGENTS.md` with app-specific status — read the relevant one before working in that app.
...
- **`spot-backend` is runnable** — use `spot-backend/AGENTS.md` + `docs/API.md`. Do not revive old “no package.json / no user_profiles” assumptions.
...
- Each `spot-*/AGENTS.md` is the source of truth for that app’s status — read it before editing.
