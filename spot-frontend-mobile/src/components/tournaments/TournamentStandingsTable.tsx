...
export default function TournamentStandingsTable({ sport, standings }: Props) {
  ...
  if (standings.length === 0) {
    return <Text style={styles.empty}>{t('tournaments.rankings.empty')}</Text>;
  }
  ...
  return (
    <View style={styles.table}>
      <View style={[styles.row, styles.headRow]}>
        ...
        <Text style={[styles.cellTeam, styles.headText]}>{t('tournaments.rankings.team')}</Text>
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  table: { backgroundColor: colors.surface, borderRadius: 14, paddingVertical: spacing.xs },
  empty: { fontSize: 13, color: colors.textMuted, textAlign: 'center', paddingVertical: spacing.lg },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  headRow: { borderBottomWidth: 1, borderBottomColor: colors.divider },
  headText: { fontSize: 11, fontWeight: '800', color: colors.textMuted },
  ptsHead: { color: colors.primary },
  cellRank: { width: 22, fontSize: 12, fontWeight: '700', color: colors.textSecondary },
  cellTeam: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  cellNum: { width: 26, textAlign: 'center', fontSize: 12, color: colors.textSecondary },
  pts: { fontWeight: '800', color: colors.textPrimary },
  logo: { width: 20, height: 20, borderRadius: 10, backgroundColor: colors.tintedSurface },
  teamName: { flexShrink: 1, fontSize: 13, fontWeight: '600', color: colors.textPrimary },
});
