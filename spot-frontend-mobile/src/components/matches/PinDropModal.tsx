...
// Ho Chi Minh City center â€” same fallback JoinMatchMapScreen uses when no
...
type Props = {
  ...
  /** Text already typed in the caller's Venue-name (or Address) field â€” seeds the search box so the host doesn't retype it. */
  seedQuery?: string;
  /** Venue name the caller already has â€” kept as the returned name unless the host picks a new search result here. */
  seedVenueName?: string;
  /** GET /geo/vn tree â€” passed down so the final pin can be matched to a Province/Ward code (see vnAdminMatch.ts). */
  ...
  onConfirm: (result: {
    ...
    longitude: number;
    address: string;
    /** Prefill for the caller's venue-name field â€” '' when nothing could be derived. */
    venueName: string;
    province?: string;
    city?: string;
  }) => void;
 ...
 * (SPOT-76 Host form Â§2). Search box (Geoapify autocomplete) jumps the map
 ...
 * was typed/selected in search â€” the host may have dragged since) to
 ...
 * (`locationLocked` stays false) â€” a wrong/missing match just means the
...
export default function PinDropModal({
  ...
  // has, then overwritten only when the host picks a search result here â€” so
  ...
  // Bumped on every open and used as PinDropMap's `key` (below) â€” forces a
  ...
  // mid-session â€” see PinDropMap.tsx) kept whatever position it was left
  ...
  // The last place the host picked from search â€” its admin fields are often
  ...
  // effect rather than <Modal onShow> â€” the latter doesn't fire reliably on
  ...
  const handleShow = () => {
    ...
    // yet â€” an already-placed pin (e.g. from a DB venue suggestion) means the
  ...
  useEffect(() => {
    ...
    // seeded query should show suggestions â€” but false when the modal opened
  ...
  function handleMove(lat: number, lng: number) {
    // Pin dragged/tapped away from the picked result â†’ its admin data no
    ...
    <Modal testID="pin-drop-modal" visible={visible} animationType="slide" onRequestClose={onCancel}>
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{t('groups.map.pinTitle')}</Text>
        ...
        <View style={styles.searchWrap}>
          <View style={styles.searchField} onLayout={(e) => setSearchFieldHeight(e.nativeEvent.layout.height)}>
            ...
            <TextInput
              ...
              placeholder={t('groups.map.addressPlaceholder')}
        ...
        <Text style={styles.helperText}>{t('groups.map.pinHelp')}</Text>
        ...
        >
          {confirming ? (
            ...
            <Text style={styles.confirmButtonText}>{t('groups.map.useLocation')}</Text>
          )}
        </TouchableOpacity>
      </SafeAreaView>
    </Modal>
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  resultText: { flex: 1, fontSize: 13, color: colors.textPrimary },

  helperText: { fontSize: 12, color: colors.textSecondary, textAlign: 'center', paddingVertical: spacing.xs },
