  ...
  return (
    <View>
      <TouchableOpacity testID={testID} style={styles.item} onPress={onToggle}>
        <Ionicons name={icon} size={20} color={themeColors.textPrimary} style={styles.itemIcon} />
...
export function ProfileMenu({ visible, onClose }: ProfileMenuProps) {
  ...
  const menuItems: MenuItemConfig[] = [
    { key: 'home', label: t('nav.home'), icon: 'home-outline', onPress: handleGoHome },
    {
      key: 'ai-assistant',
      label: t('header.aiAssistant'),
      mciIcon: 'creation',
      labelColor: themeColors.accentText,
      iconColor: themeColors.accentText,
      onPress: () => comingSoon(t('header.aiAssistant')),
    },
  ];

  const supportItems: MenuItemConfig[] = [
    { key: 'help-center', label: t('settings.helpCenter'), icon: 'help-circle-outline', onPress: () => comingSoon(t('settings.helpCenter')) },
    { key: 'contact-us', label: t('settings.contactUs'), icon: 'mail-outline', onPress: () => comingSoon(t('settings.contactUs')) },
    {
      key: 'report-issue',
      label: t('settings.reportIssue'),
      icon: 'alert-circle-outline',
      onPress: () => comingSoon(t('settings.reportIssue')),
    },
  ];
  ...
  const renderItem = (item: MenuItemConfig) => (
    <TouchableOpacity key={item.key} testID={`profile-menu-${item.key}`} style={styles.item} onPress={item.onPress}>
      {item.mciIcon ? (
        ...
        <Ionicons name={item.icon!} size={20} color={item.iconColor || themeColors.textPrimary} style={styles.itemIcon} />
      )}
            ...
            <View style={styles.card} testID="profile-menu-card">
              <TouchableOpacity testID="profile-menu-header" style={styles.header} onPress={handleOpenProfile}>
                ...
                <View style={styles.headerText}>
                  <Text style={styles.name}>{user?.fullName || t('common.guestFallback')}</Text>
              ...
              <ExpandableRow
                ...
                options={APPEARANCE_OPTIONS.map((option) =>
                  option.value === 'light'
                    ? { ...option, label: t('settings.light') }
                    : option.value === 'dark'
                    ? { ...option, label: t('settings.dark') }
                    : option
                )}
              ...
              <TouchableOpacity testID="profile-menu-sign-out" style={styles.item} onPress={handleSignOut}>
                <Ionicons name="log-out-outline" size={20} color={themeColors.error} style={styles.itemIcon} />
                <Text style={[styles.itemLabel, { color: themeColors.error, fontWeight: '600' }]}>{t('settings.signOut')}</Text>
              </TouchableOpacity>
            </View>
          </TouchableWithoutFeedback>
        </View>
...
function getStyles(c: ThemeColors) {
  return StyleSheet.create({
    ...
    header: {
      ...
      paddingBottom: 12,
      backgroundColor: c.menuHeaderBg,
      borderBottomWidth: 1,
    ...
    expandedList: {
      backgroundColor: c.menuNestedBg,
      marginHorizontal: 16,
      marginBottom: 6,
      borderRadius: 10,
      overflow: 'hidden',
