...
Reads the same Supabase Postgres database spot-backend/recommendation use
(research.md decision 7) — this service never writes to it. No new
...
built-in loader instead, matching recommendation/app/config.py.
"""
...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        model_path = os.environ.get("MODEL_PATH", "models/noshow_model.joblib")
        self.model_path = (
            Path(model_path)
            if Path(model_path).is_absolute()
            else _SERVICE_ROOT / model_path
        )
